...
typedef struct {
    ...
    XCpu_uio_map maps[ MAX_UIO_MAPS ];
} XCpu_uio_info;

/***************** Variable Definitions **************************************/
static XCpu_uio_info uio_info;
...
static int uio_info_read_name(XCpu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCpu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCpu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCpu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCpu_Initialize(XCpu *InstancePtr, const char* InstanceName) {
	XCpu_uio_info *InfoPtr = &uio_info;
